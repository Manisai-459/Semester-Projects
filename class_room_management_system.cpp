...
string is_there(string log, string name)
{
    ifstream file(log);
    if (file)
    {
...
int slot_display(string x, int p = 0)
{
    string y;
    int count = 0;
    ifstream file(x + ".txt");
...
public:
    ...
    void book_slot(string x, int s, int e, string room_no)
    {
        name = x;
        ...
        file << to_string(s) + "-" + to_string(e) + " " + sub + " " + sec + " " + room_no << endl;
        m << to_string(s) + "-" + to_string(e) << " " << room_no << " " << name.substr(0, name.find('.')) << " " << sub << endl;
        file.close();
        m.close();
        vacant = 0;
    ...
    if (x != "-")
    {
        ...
        do
        {
            ...
            case 2:
            {
                ...
                {
                    ofstream file("password_log.txt", ios::app);
                    file << name + "@srmap.edu.in" + " " + name + "@srm123" << endl;
                    file.close();
                }
                else
...
int main()
{
    ...
    cout << "\t\t\t - Email ID must be of the form xxxxxxx@srmap.edu.in" << endl;
    cout << "\t\t\t - Password must contain minimum 8 character length" << endl;
    cout << "\t\t\t - default password for all new lecturers is username@srm123" << endl;
    cout << "\t\t\t - Collage block consists of 5 floors , each floor consists of 10 rooms" << endl;
    cout << "\t\t\t - if it is 1st floor then room number are 101 to 110 and follows" << endl;
    cout << "\t\t\t - there are 8 time slots available" << endl;
    cout << "\t\t\t\t  9 to 10 \n\t\t\t\t  10 to 11 \n\t\t\t\t  11 to 12 \n\t\t\t\t  12 to 1 \n\t\t\t\t  1 to 2 \n\t\t\t\t  2 to 3 \n\t\t\t\t  3 to 4 \n\t\t\t\t  4 to 5 " << endl;
    cout << "\t\t\t - at a time only 1 lecture hour can be assigned i.e.. form  ( 9-10 ) or ( 1-2 ) or ( 10-11 )\n " << endl;
    do
    {
        cout << "\t\t\t\t ======================================================================" << endl;
